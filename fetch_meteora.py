"""
Script to fetch trading pairs from Meteora - testing various endpoints
"""
import requests
import json
from typing import List, Dict

def test_meteora_endpoints():
    """Test various possible endpoints for Meteora API"""
    base_url = "https://app.meteora.ag/api"
...
if __name__ == "__main__":
    ...
    data, endpoint = test_meteora_endpoints()
    
    if data:
        print(f"\nSuccessfully fetched data from {endpoint}")
        print(f"Data preview: {json.dumps(data, indent=2)[:500]}")
        
        with open('meteora_data.json', 'w') as f:
            json.dump(data, f, indent=2)
        print(f"\nData saved to meteora_data.json")
    else:
